...
use crate::copy::{Reflink, SourceSpec, Sparse};

// ---------------------------------------------------------------------------
// Énumérations et structures d'options
// ---------------------------------------------------------------------------
...
impl Preserve {
    ...
    fn set(&mut self, attr: &str, value: bool) -> Result<()> {
        match attr.trim() {
...
impl Args {
    ...
    pub fn resolve(&self) -> Result<Config> {
        ...
        let bwlimit = match &self.bwlimit {
            Some(s) => Some(
                crate::utils::parse_rate(s).map_err(|e| anyhow::anyhow!("--bwlimit : {e}"))?,
            ),
            None => None,
        };
